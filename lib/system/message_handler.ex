    ...
    quote do
      ...
      defp with_new_aggregate(log_msg, cmd, id \\ nil, fun) do
        ...
        with {:ok, pid} <- spawn_new(key),
             ...
             {:ok, last_event} <- apply_changes(pid, key, transaction, events, version) do
      ...
      defp with_aggregate(log_msg, id, fun) do
        ...
        with {:ok, pid} <- get_pid(id),
             {:ok, transaction, events, version, aggr_resp} <- fun.({:ok, pid}) do
      ...
      defp get_pid(id),
        do:
          PidFacade.get_pid(
            @pid_facade,
            id,
            when_pid_is_not_registered(),
            &aggregate_start_params/1
          )

      defp exit_process(id, reason \\ :normal),
        do: PidFacade.exit_process(@pid_facade, id, reason)

      defp delete_stream(key, soft_or_hard, expected_version \\ -2)
      ...
      defp apply_changes(aggregate, _, transaction, [], expected_version) do
        {:ok, _} =
          aggregate_mod().commit(aggregate, transaction, expected_version, expected_version)
      ...
      defp apply_changes(aggregate, key, transaction, events, expected_version) do
        case save_events(key, events, expected_version) do
          {:ok, last_event_number} ->
            {:ok, new_state} =
              aggregate_mod().commit(aggregate, transaction, expected_version, last_event_number)
      ...
      defoverridable save_events: 3,
                     save_state: 2,
                     when_pid_is_not_registered: 0,
                     aggregate_start_params: 1
    end
  end
end
