defmodule Extreme.System.EventStore do
  ...
  #        |> Enum.each(fn {aggregate_group, stream} ->
    #             quote do
      ...
      def init({config_module, extreme}), do: {:ok, %{config: config_module, extreme: extreme}}
  ...
  def handle_call({:save_events, {identifier, events, metadata, expected_version}}, _from, state) do
    ...
    case Extreme.execute state.extreme, write_events(stream, events, metadata, expected_version) do
      ...
      {:error, :WrongExpectedVersion, %Extreme.Msg.WriteEventsCompleted{commit_position: current_version}} -> 
        {:reply, {:error, :wrong_expected_version, expected_version, current_version}, state}
      other ->
