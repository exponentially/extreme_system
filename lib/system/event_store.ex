defmodule Extreme.System.EventStore do
  ...
  def handle_call({:delete_stream, identifier, hard_delete?, expected_version}, _from, state) do
    ...
    response =
      case Extreme.execute(state.extreme, delete_stream(stream, hard_delete?, expected_version)) do
        {:ok, _} -> :ok
        {:error, :NoStream, _} -> :ok
      end
  ...
  defp fetch_stream_events({extreme, stream, start_at, per_page, _is_completed}) do
    ...
    {events, is_end_of_stream} =
      case Extreme.execute(extreme, read_events(stream, start_at, per_page)) do
        {:ok, response} ->
          events =
            Enum.map(response.events, fn e ->
              event =
                Poison.decode!(e.event.data,
                  as: struct(String.to_atom(e.event.event_type)),
                  keys: :atoms
                )
